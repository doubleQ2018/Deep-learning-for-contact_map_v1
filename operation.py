...
def res_block_1d(incoming, out_channels, filter_size, regularizer, batch_norm=False,  block_name='BLOCK_1D'):
    net = incoming
    in_channels = incoming.get_shape().as_list()[-1]
    ident = net
    # 1st conv layer in residual block
    W1 = weight_variable([filter_size, in_channels, out_channels], regularizer, block_name+'_W1')
    #variable_summaries(W1)
    b1 = bias_variable([out_channels], 'b1')
    ...
    W2 = weight_variable([filter_size, out_channels, out_channels], regularizer, block_name+'_W2')
    #variable_summaries(W2)
    b2 = bias_variable([out_channels], 'b2')
...
def res_block_2d(incoming, out_channels, filter_size, regularizer, batch_norm=False, block_name='BLOCK_2D'):
    net = incoming
    in_channels = incoming.get_shape().as_list()[-1]
    ident = net
    # 1st conv layer in residual block
    W1 = weight_variable([filter_size, filter_size, in_channels, out_channels], regularizer, block_name+'_W1')
    #variable_summaries(W1)
    b1 = bias_variable([out_channels], 'b1')
    #variable_summaries(b1)
    ...
    net = tf.nn.relu(net)
    ### 2nd conv layer in residual block
    W2 = weight_variable([filter_size, filter_size, out_channels, out_channels], regularizer, block_name+'_W2')
    #variable_summaries(W2)
    b2 = bias_variable([out_channels], 'b2')
    #variable_summaries(b2)
